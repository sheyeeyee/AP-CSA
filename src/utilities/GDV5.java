...
public abstract class GDV5 extends Canvas implements Runnable, KeyListener {
	...
	private static int pongX = 1200;
	private static int pongY = 800;
	private static int breakoutX = 1200;
	private static int breakoutY = 800;
	private static int snakeX = 600;
	private static int snakeY = 600;
	private static int MAX_WINDOW_X = snakeX;
	private static int MAX_WINDOW_Y = snakeY;
	private static int PADDING = 2;

	// it is your responsibility to handle the release on keysTyped
	public static boolean[] KeysTyped;
	private JFrame frame;
	private String title = "Snake: MMORPG";
...
//		else if (BreakoutRunner.getGameStart() == false) this.setBackground(background);
	...
	public void start() {
		...
		frame.add(this);
		frame.pack();
		frame.setTitle(title); //calls it "Pong: MMORPG"
	...
	public abstract void update(); //abstract methods, undefined here, so the "children" define them
	...
	//need to copy paste this and change it to a different method that measures not just the top left corner of the paddle
